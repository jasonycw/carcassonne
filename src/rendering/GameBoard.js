...
// ---------------------------------------------------------------------------
// drawScoreboard  (internal)
// ---------------------------------------------------------------------------

/**
 * Render the player scoreboard as SVG text / images inside the provided
 * score-area group.  Idempotent via D3 data-join on score-group elements.
 */
function drawScoreboard(container, gamestate, reorderedPlayers, callbacks) {
  let scoreBoardX = 5; // running horizontal offset (in SVG px)

  // ── Player rows ───────────────────────────────────────────────────────
  const scoreGroups = container.selectAll('g.score-group')
    .data(reorderedPlayers, (d) => d.user._id || d.color);

  const scoreEnter = scoreGroups.enter().append('g')
    .attr('class', 'score-group');

  // Point total (right-most column).
  scoreEnter.append('text')
    .attr('class', 'point-total')
    .attr('text-anchor', 'end');

  // Player name outline (for readability).
  scoreEnter.append('text')
    .attr('class', 'player-name-outline')
    .attr('text-anchor', 'start')
    .attr('stroke', 'black')
    .attr('stroke-width', 2.5);

  // Player name (coloured fill).
  scoreEnter.append('text')
  ...
  scoreGroups
    .attr('id', (d) => `score-group-${d.color}`);

  // Find the player's index in the ORIGINAL (non-reordered) players array
  // so we can use it for meeple image paths.
  const getOriginalIndex = (player) => {
    const orig = gamestate.players || [];
    for (let i = 0; i < orig.length; i++) {
      if (orig[i].user._id === player.user._id) return i;
    }
    return 0;
  };

  scoreGroups.each(function (player, i) {
    const g = d3.select(this);
    const yBase = TILE_SIZE / 8 + i * (TILE_SIZE / 2 - 5);

    // Points.
    g.select('text.point-total')
      .attr('x', 5 + 7 * (TILE_SIZE / 4.75))
      .attr('y', yBase)
      .text(player.points);

    // Player name (outline + fill).
    g.select('text.player-name-outline')
      .attr('x', 5)
      .attr('y', yBase)
      .text(player.user.username);

    g.select('text.player-name')
      .attr('x', 5)
      .attr('y', yBase)
  ...
  reorderedPlayers.forEach((player, i) => {
    ...
    normalMeepleData.reverse(); // right-to-left stacking
  });

  const normalMeeples = container.selectAll('image.normal-meeple')
    .data(normalMeepleData);

  normalMeeples.join(
    (enter) => enter.append('image')
      .attr('class', 'normal-meeple')
      .attr('width', TILE_SIZE / 4)
      .attr('height', TILE_SIZE / 4)
      .on('click', function (event, d) {
        if (d.row === 0 && callbacks.onScoreboardClick) {
          callbacks.onScoreboardClick(d.playerIndex);
        }
  ...
  // Update running X for special pieces.
  scoreBoardX = 5 + 6 * (TILE_SIZE / 4.75) + TILE_SIZE / 4;

  // ── Expansion pieces ──────────────────────────────────────────────────
  const expansions = gamestate.expansions || [];

  // Large meeple (Inns & Cathedrals).
  if (expansions.indexOf('inns-and-cathedrals') !== -1) {
    ...
    drawExpansionMeeple(container, 'large-meeple', largeData, TILE_SIZE * 3 / 8, callbacks);
    if (reorderedPlayers.length > 0) {
      scoreBoardX += TILE_SIZE * 3 / 8;
    }
  }

  // Pig meeple (Traders & Builders).
  if (expansions.indexOf('traders-and-builders') !== -1) {
    ...
    drawExpansionMeeple(container, 'pig-meeple', pigData, TILE_SIZE * 3 / 8, callbacks);
    if (reorderedPlayers.length > 0) {
      scoreBoardX += TILE_SIZE * 3 / 8;
    }
  }

  // Builder meeple (Traders & Builders).
  if (expansions.indexOf('traders-and-builders') !== -1) {
    const builderData = [];
    reorderedPlayers.forEach((player, i) => {
    ...
    drawExpansionMeeple(container, 'builder-meeple', builderData, TILE_SIZE * 3 / 8, callbacks);
    if (reorderedPlayers.length > 0) {
      scoreBoardX += TILE_SIZE * 3 / 8;
    }
  }

  // Tower meeples (The Tower expansion).
  if (expansions.indexOf('the-tower') !== -1) {
    ...
    drawTowerMeeple(container, towerData, TILE_SIZE * 3 / 8, callbacks);
    if (reorderedPlayers.length > 0) {
      scoreBoardX += TILE_SIZE * 3 / 8;
    }
  }
  ...
  if (expansions.indexOf('traders-and-builders') !== -1) {
    ...
    drawGoodsTokens(container, goodsData, largeSize);
    if (reorderedPlayers.length > 0) {
      scoreBoardX += 3 * largeSize;
    }
  }

  // ── Score background resize ───────────────────────────────────────────
  container.select('rect.score-bg')
    .attr('height', TILE_SIZE / 4 + reorderedPlayers.length * (TILE_SIZE / 2 - 5))
    .attr('width', scoreBoardX + 20)
    .attr('opacity', 0.75);
}

/** Helper: draw a single expansion meeple type (large, pig, builder). */
function drawExpansionMeeple(container, className, data, size, callbacks) {
  const sel = container.selectAll(`image.${className}`).data(data);
  sel.join(
    (enter) => enter.append('image')
...
/** Helper: draw tower meeple with count label. */
function drawTowerMeeple(container, data, size, callbacks) {
  const sel = container.selectAll('g.tower-meeple-group').data(data);
  const enter = sel.enter().append('g').attr('class', 'tower-meeple-group');
  enter.append('image')
    .attr('class', 'tower-meeple')
    .attr('width', size)
    .attr('height', size)
    .on('click', function (event, d) {
      if (d.row === 0 && callbacks.onScoreboardClick) {
  ...
  sel.select('image')
    ...
    .attr('href', (d) => d.url);
  sel.select('text')
    .attr('x', (d) => d.x + size / 2 + 13)
    .attr('y', (d) => d.y + 7)
    .text((d) => d.count);
}

/** Helper: draw goods token icons with count labels. */
function drawGoodsTokens(container, data, size) {
  const sel = container.selectAll('g.goods-token-scores').data(data);
  const enter = sel.enter().append('g').attr('class', 'goods-token-scores');
  enter.append('image')
    .attr('width', size)
    .attr('height', size);
  enter.append('text')
...
export function clearBoard() {
  ...
  if (scoreArea) {
    scoreArea.selectAll('g.score-group').remove();
    scoreArea.selectAll('image.normal-meeple').remove();
    scoreArea.selectAll('image.large-meeple').remove();
    scoreArea.selectAll('image.pig-meeple').remove();
    scoreArea.selectAll('image.builder-meeple').remove();
    scoreArea.selectAll('g.tower-meeple-group').remove();
    scoreArea.selectAll('g.goods-token-scores').remove();
  }
  lastGamestate = null;
  lastPlayerId = null;
  lastCallbacks = null;
